...
> v3 기획 중 Lv6·9 분기 진화/가지 않은 길 타락체 보스는 **구현되지 않았고 채택하지 않는다.**
> (단, 표식 반응/시너지는 이후 사용자 요청으로 채택됨 — `systems/reactions.ts` 참조.)
...
5. **★표식(Mark) 시스템 + 반응(시너지)**: 모든 속성 공격은 `Marks`를 남긴다. 표식은 **역할이 뚜렷이 분리된 단독 효과**다 — 🔥화상=지속피해, 💧젖음=가벼운 감속, 🍃**덩굴=하드 속박(강감속+주기적 뿌리, DoT 아님)**, 🌑저주=받는피해 증폭, 🌟축복=아군 공버프. **표식 반응(시너지)은 `systems/reactions.ts`의 단일 판정 테이블로 존재한다(사용자 요청으로 도입)**: 서로 다른 속성 표식이 같은 적에 겹치면 반응이 1회 터지고 두 표식을 소모(증기폭발/들불/얽힘의 저주/침식). 모든 표식 적용은 `Battle.applyMark()` 한 경로를 거친다 — **조합별 하드코딩 금지, 반응 추가는 REACTIONS 테이블에만.**
...
- **육성**: 레벨업 → 카드 해금, 진화(불/물/풀 Lv6·16, 빛/어둠 Lv10·18, 대기만성 ×0.88→3단 점프). 포획 적은 쌍 라인이면 Lv10에 2단 진화. (실제 값은 `monsters.csv`·`ENEMY_EVOLVE_LEVEL`이 정본. XP 곡선상 최종진화 ≈ 스테이지 7~8)
- **역할 분리(크리처 vs 포획 적)**: 크리처 = **주문 캐스터** — 깊고 값비싼 고유 속성 스킬덱(`fire/water/...`)이 전략의 핵심 화력. 포획 적 = **전선 바디** — 크리처보다 확연히 단단한 HP(`PLAY_BASE_HP`)와 tier 패시브로 벽·제어를 맡고, 스킬은 저코스트 유틸 3종(`e_*`, 값싼 공격·표식 + Lv10 각성 시그니처)만 갖는다. → "왜 포획 vs 왜 크리처"가 명확하고, 덱은 크리처 고코스트 주문 위주라 **마나 운영 압박**이 전략축이 된다.
...
- [x] **포획 시스템** (포획구 카드 → 오브 투척 → 반경 판정) + 포획 도감. **보스/미니보스는 포획 불가** — HP0에서 즉사 대신 잠깐 기절(비틀거림) 연출만 남기고 소멸(`CAPTURE.bossStun`).
...
- **포획이 핵심 루프.** 포획 판정 반경(`CAPTURE_RADIUS`)·원정대 상한(`MAX_MONSTERS`)은 밸런스 민감. 함부로 바꾸지 말 것. 보스/미니는 포획 대상이 아니다(`CAPTURE.bossStun`은 기절 연출 시간).
- **표식 반응(시너지)은 채택된 기능이다.** 반응 규칙/피해는 `systems/reactions.ts`의 `REACTIONS` 테이블 + `constants.REACTION`에서만 조정. 새 조합은 테이블에 추가하고 `Battle.applyMark()` 단일 경로를 유지할 것(분기 하드코딩 금지).
- 분기 진화/타락체 보스는 **명시적 요청이 없는 한 새로 넣지 말 것** (v3 잔재).
